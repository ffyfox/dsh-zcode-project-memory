...
本报告逐项判定 `dsh-project-memory` 是否符合 dsh 官方开发文档规范。
...
| R7 | `publish.md` L56「The patch is a YAML array … plugin rows reference the package **by name** instead of a relative source path so Node resolution finds the installed code」 | `cordis.patch.yml` L12-14：YAML 数组，`name: dsh-project-memory` 与 `package.json` L2 的包名一致 | 符合 |
